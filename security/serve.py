...
- **outputSchema strip** — always applied to locally-registered tools (a FastMCP-3.x
  connector-compat fix; see :func:`_strip_local_output_schemas`). ``from_openapi`` tools
  must strip at build time instead.
...
def _strip_local_output_schemas(mcp) -> None:  # type: ignore[no-untyped-def]
    ...
    FastMCP 3.x derives an ``outputSchema`` for every tool (even plain ``-> str`` ones);
    Claude's connector then drops the per-tool approval toggle (and sometimes the tool)
    from connector settings (anthropics/claude-code#25081). Nulling it makes the wire
    shape match what the connector handles -- verified at the HTTP layer: tools/list
    omits outputSchema after this runs.
    ...
    shape changes. Escape hatch: ``MCP_KEEP_OUTPUT_SCHEMA=1``.
    """
